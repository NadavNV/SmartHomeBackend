    ...
    if cls == str:
        if type(value_range) is set:
            if value not in value_range:
                app.logger.error(f"{name} must be one of {value_range}, got {value} instead.")
            ...
            return (bool(re.match(TIME_REGEX, value)),
                    None if re.match(TIME_REGEX, value) else f"'{value}' is not a valid hex color string.")
    return True, None
...
def validate_device_data(new_device) -> tuple[bool, str | None]:
    ...
    for field in list(new_device.keys()):
        if field == 'type' and new_device['type'] not in DEVICE_TYPES:
            app.logger.error(f"Incorrect device type {new_device['type']}, must be one of {DEVICE_TYPES}.")
